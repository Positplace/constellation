...
export const MoonDetailsCard: React.FC<MoonDetailsCardProps> = ({
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[60] bg-black/80 backdrop-blur-sm border border-white/20 rounded-lg w-96 max-h-[70vh] text-white shadow-2xl pointer-events-auto flex flex-col">
      {/* Close Button - Sticky */}
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 text-gray-400 hover:text-white transition-colors bg-black/80 rounded-full w-6 h-6 flex items-center justify-center"
        >
          ✕
        </button>
      )}
      ...
      <div className="overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent p-4">
        ...
        <div className="mb-4 pr-8">
          ...
          <p className="text-sm text-gray-400">Moon of {parentPlanetName}</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
            ...
            <div>
              {moon.rotation.direction === 1 ? "Prograde" : "Retrograde"}
            </div>
          </div>
        </div>
            ...
            <div
              ...
              style={{
                backgroundColor:
                  moon.type === "ice_world"
                    ? "#E6F3FF"
                    : moon.type === "rocky_world"
                    ? "#8B7355"
                    : moon.type === "terrestrial"
                    ? "#4A5D23"
                    : moon.type === "desert_world"
                    ? "#D2B48C"
                    : moon.type === "dwarf_planet"
                    ? "#696969"
                    : moon.type === "lava_world"
                    ? "#FF4500"
                    : moon.type === "ocean_world"
                    ? "#4682B4"
                    : moon.type === "frozen_world"
                    ? "#B0E0E6"
                    : "#8B7355",
              }}
            />
